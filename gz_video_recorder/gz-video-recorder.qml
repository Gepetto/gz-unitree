...
ToolBar {
  ...
  RowLayout {
    ...
    FileDialog {
        ...
        onAccepted: {
          ...
          if (fileFormat == fileUrl.toString()) {
            // no format specified
            RemoteVideoRecorder.OnSave(fileFormat + "." + selectedFormat)
            close()
          }
            ...
            RemoteVideoRecorder.OnSave(fileUrl)
            close()
          }
        }
        onRejected: {
          RemoteVideoRecorder.OnCancel()
          close()
        }
    }
    ...
    Dialog {
      ...
      onAccepted: {
        RemoteVideoRecorder.OnSave(fileDialog.fileUrl)
        RemoteVideoRecorder.close()
        close()
      }
      onRejected: {
        fileDialog.open()
    ...
    Menu {
      ...
      MenuItem {
        ...
        onTriggered: {
          ...
          RemoteVideoRecorder.OnStart("mp4")
          animation.start()
        }
      }
      MenuItem {
        ...
        onTriggered: {
          ...
          RemoteVideoRecorder.OnStart("ogv")
          animation.start()
        }
      }
    }
